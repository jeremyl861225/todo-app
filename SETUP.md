# 待辦事項 App — 設定說明

一個可以放在手機主畫面、手機與電腦共用同一份資料的待辦事項程式。

- **頁面一「重複排程」**：每天／每週／每兩週／每月要做的事，可新增、編輯、刪除。
- **頁面二「月曆」**：與頁面一同步，可勾選完成、加入單一行程；**不能**在這裡修改或刪除重複排程。
- **頁面三「近期任務」**：一次性的任務，可拆子任務、依截止日分組；**不會**出現在月曆。
- 重複排程與單一行程也可以拆**子事項**。差別在勾選是**每一次各自記錄**的：
  「晨會簡報」今天把兩項都勾完，明天再看會重新開始——跟排程本身的完成同一個道理。
  任務的子任務則是整筆共用一份（任務只發生一次）。
  勾完成後在清單再留兩天，之後移入右上角的「已完成」。

---

## ✅ 目前狀態（2026-08-23）

**已經可以直接使用**：<https://jeremyl861225.github.io/todo-app/>

已完成：Supabase 專案 `dawcpdgonxmhojwonkut` 建立、資料表與 RLS 建好、
金鑰已填進 `index.html`、部署到 GitHub Pages。

**migration 全部執行完畢**（到 Supabase → SQL Editor 執行 `migrations/` 底下的檔案；
App 啟動時會偵測欄位，缺哪一份就在頂端指名顯示）：

| 檔案 | 內容 | 狀態 |
|---|---|---|
| [`2026-08-recurrence.sql`](migrations/2026-08-recurrence.sql) | 排程起訖日、「自訂」頻率、單一行程多日活動 | ✅ 已執行 |
| [`2026-08-important.sql`](migrations/2026-08-important.sql) | 重大事件標記 | ✅ 已執行 |
| [`2026-08-monthly-nth.sql`](migrations/2026-08-monthly-nth.sql) | 每月「第幾個星期幾」 | ✅ 已執行 |
| [`2026-08-tasks.sql`](migrations/2026-08-tasks.sql) | 近期任務（`tasks` 資料表、子任務、完成日期） | ✅ 已執行 2026-08-23 |
| [`2026-08-move.sql`](migrations/2026-08-move.sql) | 單次挪動排程日期 | ✅ 已執行 |
| [`2026-08-subitems.sql`](migrations/2026-08-subitems.sql) | 重複排程與單一行程的子事項（`sub_done` 資料表，逐日勾選） | ✅ 已執行 2026-08-31 |

`tasks` 建好後已用真實 RLS 條件驗過（交易內測試、已 rollback 不留資料）：
本人可新增／讀取／更新，另一個帳號讀 0 筆、改 0 筆、刪 0 筆，未登入讀 0 筆。

`sub_done` 同樣驗過（同一套方法）：本人可新增並讀到 1 筆，另一個帳號讀 0 筆、
改 0 筆、刪 0 筆，未登入讀 0 筆；測試資料已隨交易 rollback，資料表為空。

> ⚠️ Supabase 的 **Confirm email 仍是開啟的**，所以新註冊的帳號要去信箱點確認信才能登入。
> 想省掉這一步，到 Supabase → Authentication → Sign In / Providers → Email →
> 把 **Confirm email** 關掉並 Save（詳見下面步驟三）。

以下步驟是**重新部署一份**或日後要自己重建時才需要看的紀錄。

---

## 步驟一：建立 Supabase 專案

1. 到 <https://supabase.com> 註冊（可用 GitHub 帳號）。
2. 點 **New project**：
   - Name：隨意，例如 `todo`
   - Database Password：自訂一組（之後用不到，但請存起來）
   - Region：選 **Northeast Asia (Tokyo)** 或 Singapore，速度較快
3. 按 **Create new project**，等 1–2 分鐘建置完成。

## 步驟二：建立資料表

1. 左側選單 → **SQL Editor** → **New query**。
2. 把本資料夾裡 [`schema.sql`](schema.sql) 的內容 **全部貼上**。
3. 按 **Run**。看到 `Success` 就完成了。

> 這段 SQL 可以重複執行，不會刪掉既有資料。
> 它也一併開啟 Row Level Security，確保每個帳號只讀得到自己的資料。

## 步驟三：關掉「Email 確認信」（建議）

1. 左側 → **Authentication** → **Sign In / Providers** → **Email**。
2. 把 **Confirm email** 關掉 → **Save**。

> 不關也可以，只是註冊後要先去收信點連結才能登入。

## 步驟四：把金鑰填進程式

1. 左側 → **Project Settings**（齒輪）→ **API Keys**。
2. 複製兩個值：
   - **Project URL**，長得像 `https://abcdefgh.supabase.co`
   - **anon public** key，很長的一串 `eyJ...`
3. 用文字編輯器打開 `index.html`，找到最上方這一段（約在 `<script>` 開頭）：

```js
const CONFIG = {
  SUPABASE_URL: '',
  SUPABASE_ANON_KEY: ''
};
```

把兩個值填進去：

```js
const CONFIG = {
  SUPABASE_URL: 'https://abcdefgh.supabase.co',
  SUPABASE_ANON_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9....'
};
```

> **anon key 可以公開嗎？** 可以。它本來就設計成放在前端網頁裡，
> 真正的權限由步驟二設定的 Row Level Security 控制——沒有登入就什麼都讀不到，
> 登入後也只讀得到自己的資料。

（若你不想改檔案，也可以直接開啟網頁，程式會顯示設定畫面讓你貼上這兩個值，
但那樣**每一台裝置都要各貼一次**。填進 `index.html` 比較省事。）

---

## 步驟五：放到網路上

因為要在手機開，檔案必須放在一個 `https://` 網址上。最簡單的是 GitHub Pages：

1. 開一個新的 GitHub repository（public 或 private 皆可，Pages 需 public 或付費方案）。
2. 把這個資料夾裡的所有檔案上傳到 repo 根目錄。
3. repo → **Settings** → **Pages** → Source 選 `main` 分支、`/ (root)` → Save。
4. 等一兩分鐘，網址會是 `https://<你的帳號>.github.io/<repo 名稱>/`。

> 提醒：你的 `xxx.github.io` 底下已經有其他 PWA。這個 App 的 `sw.js` 刻意
> **不建立任何快取**，因此不會影響到 Clinical-Tools 或題庫 App 的離線快取。

## 步驟六：加到手機主畫面

> 目前的網址：**<https://jeremyl861225.github.io/todo-app/>**


**iPhone（Safari）**
1. 用 Safari 開上面的網址（Chrome 不行，iOS 只有 Safari 能加入主畫面）。
2. 下方「分享」→ **加入主畫面** → 加入。

**Android（Chrome）**
1. 開網址 → 右上角「⋮」→ **加到主畫面 / 安裝應用程式**。

---

## icon

目前用的是 `task-list.png`，底色 `#EEEEEE`、黑色線條，比例與 Clinical-Tools、每日文獻一致。

| 檔名 | 尺寸 | 圖案佔比 | 用途 |
|---|---|---|---|
| `icon-180.png` | 180×180 | 58.9% | iPhone 主畫面 |
| `icon-192.png` | 192×192 | 58.3% | Android／瀏覽器分頁 |
| `icon-512.png` | 512×512 | 58.2% | Android 啟動畫面 |
| `maskable-512.png` | 512×512 | 36.3% | Android 自動裁形（圓形／方形）用 |

> 「圖案佔比」是黑色圖形的外框相對於畫布的邊長比。一般 icon 一律 **298/512**（左右各留 107px），
> maskable 因為系統會裁掉外圈，縮到 **186/512**（左右各留 163px）。這是 Clinical-Tools
> 與每日文獻沿用的規格，要再換圖時照這個比例縮放就會一致。

日後換圖的做法（來源圖需為 512×512、底色 #EEEEEE、圖案置中）：

```bash
# 先量出來源圖黑色圖形的外框寬度，設為 INK
# 一般 icon：整張縮到 512 × (298/INK)，再補白回 512
sips -z $((512*298/INK)) $((512*298/INK)) task-list.png --out /tmp/m.png
sips -p 512 512 --padColor EEEEEE /tmp/m.png --out icon-512.png
sips -z 192 192 icon-512.png --out icon-192.png
sips -z 180 180 icon-512.png --out icon-180.png
# maskable：把 298 換成 186 重做一次
```

換完重新整理即可（iPhone 需移除主畫面圖示再重新加入）。

---

## 使用說明

### 頁面一：重複排程
- 右下角 **＋ 新增排程**。
- **起始日必填**（預設當天），這天以前不會出現。**結束日選填**，留空表示持續進行。
  已過結束日的排程會淡化並標「已結束」，但不會被刪掉。
- 頻率四選一：
  - **每日** — 每天
  - **每週** — 選星期幾
  - **每月** — 兩種指定方式擇一：
    - **幾號**：選 1–31 號；若當月沒有那天（例如 31 號遇到小月），會自動落在該月最後一天
    - **第幾個星期幾**：例如「第二個」＋「三」＝每月第二個星期三。
      序數可選第一～第四個或**最後一個**（用「最後一個」就不會有某些月份找不到第五個的問題）
  - **自訂** — 每 N 天／每 N 週（選星期幾）／每 N 個月（選幾號），一律**從起始日起算**。
    例如起始日 8/3、每 2 週的週一 → 8/3、8/17、8/31…
- 「時間」是自由文字，可以寫 `08:00`，也可以寫「晨會後」。
- 可加任意多筆「相關連結」，自訂名稱＋網址（網址不打 `https://` 也會自動補）。
- 點任一張卡片看完整內容，裡面有「編輯」與「刪除」。

### 頁面二：月曆
- 點日期看那天所有事項；每項左邊的方框可勾選完成，**只影響那一天**。
- 點事項名稱看完整內容。重複排程在這裡只能勾完成，要改內容請回頁面一。
- 右下角 **＋ 新增行程** 可加入單一行程（例如某天的演講、考試）。
  填了「結束日期」就是**跨多日活動**，例如 8/12–8/14 的研討會。
- 單一行程在月曆頁可以直接編輯與刪除。

**多日行程用一條橫桿橫跨那幾天**，不是每天各一塊。跨週時會在週界切斷，
被切斷的那一端切平並加上「▸」，看得出是同一條延續下去。同一週有多筆重疊時會自動分層。
橫桿要**那一段涵蓋的每一天都勾完**才會變成已完成的樣式。點橫桿會跳到對應那天並打開內容。

**三種項目在月曆上長得不一樣**：多日行程是橫桿，單日的單一行程是圓角描邊色塊（手機是菱形點），
重複排程是灰底加左側色條（手機是圓點）。月曆下方有圖例。

### 頁面三：近期任務
- 右下角 **＋ 新增任務**。名稱必填，其餘（截止日、類別、子任務、敘述、連結）都是選填。
- 依截止日自動分組：**已逾期 / 今天到期 / 一週內 / 之後 / 未定截止日**。
  同一組裡標了重大事件的排最前面，其餘按截止日先後。
- **子任務**：卡片上一個子任務一格綠色小方塊，完成就亮；標題下的小灰字是
  **第一個還沒完成的子任務**，一眼就知道下一步要做什麼。子任務的勾選在卡片的詳細內容裡，
  新增／改名／刪除／**排序**在編輯畫面。
- **調整子任務順序**：在編輯畫面按住每一列左邊的**握把**（六個點）上下拖，放開就定位，
  按「儲存變更」才寫入。手機直接用手指拖，桌機用滑鼠；也可以先點一下握把再用 **↑ ↓** 鍵搬。
  順序改了之後，卡片上的小方塊、「第一個未完成的子任務」都會跟著換。
- 勾完成（卡片左邊的方框，或詳細內容裡的「標記為完成」）會**記下完成日期**，
  卡片先留在「剛完成」這一組，**滿兩天**才移到「已完成」頁。
  取消勾選再勾一次，完成日期會換成新的，兩天**重新算起**。
- 右上角 **已完成** 進入歸檔頁（依完成月份分組），左上角的按鈕回到近期任務。
  在歸檔頁取消勾選，任務就會回到近期任務。
- **任務不會出現在月曆**，月曆只放重複排程與單一行程。

### 匯入排程
右上角「⋮」→ **匯入 JSON** → **從檔案選擇**。可匯入：

- 自己「匯出備份 JSON」存下來的檔案，等同還原備份
- 事先轉好格式的排程檔，格式為
  `{"schedules":[…],"events":[…],"tasks":[…],"categories":[{"name":"…"}]}`（欄位同匯出備份）

匯入前會列出每一筆讓你確認。項目依內部編號覆蓋，**重複匯入同一個檔案不會產生重複資料**，
所以檔案更新後再匯入一次就是更新內容。

> ⚠️ **排程檔不要放進這個 repo**。repo 是 public，放進去等於公開檔案內容——
> 職務排程通常含同事姓名、內部 Google 文件連結等不該外流的資訊。
> `.gitignore` 已擋掉 `imports/`。檔案自己保管，用「從檔案選擇」匯入即可。

### 重大事件
重複排程、單一行程與任務的編輯畫面裡都有「**標記為重大事件**」開關。標記後：

- 清單上加星號、外框變成暖色，並**排在同一個頻率分組的最前面**
- 月曆的方塊與手機的圓點都加上星號／星環
- 詳細內容最上方會有「★ 重大事件」標籤
- 頁面一的篩選列多一顆「★ 重大」，可只看重大事件（能和類別一起用）

### 密碼
- **更改密碼**：右上角「⋮」→ 更改密碼。要輸入目前的密碼確認是本人
  （避免手機被別人拿去直接改掉），改完其他裝置仍保持登入。
- **忘記密碼**：登入頁下方的「忘記密碼？」→ 填 Email → 寄出重設信 →
  點信裡的連結會回到這個頁面，直接設定新密碼。

> ⚠️ 重設信的連結要能正確回到 App，必須在 Supabase 設定允許的網址：
> **Authentication → URL Configuration**，把 Site URL 設成
> `https://jeremyl861225.github.io/todo-app/`，並在 Redirect URLs 也加同一個網址。
> 沒設定的話，點信裡的連結會被導去別的地方（預設是 `http://localhost:3000`）。

### 類別
右上角「⋮」→ **管理類別**。新帳號預設有「工作」「私人」，可自行增刪。
新增排程時在類別下拉選單選「＋ 新增類別…」也能直接加。

---

## 離線可以用嗎

可以。**App 本體**（網頁、程式庫、圖示）會快取在手機上，沒網路也開得起來、不用每次重抓，
所以第二次之後開啟是秒開的。**資料**也有一份在手機本機，離線時看到的是最後一次同步的內容。

離線時能做什麼：

| 動作 | 離線可否 |
|---|---|
| 開啟 App、看排程、月曆與任務 | ✅ |
| 新增／編輯／刪除、勾選完成 | ✅ 存在本機，右上角顯示「待上傳 N」，恢復連線自動補送 |
| 登入、註冊、改密碼 | ❌ 需要連線 |
| 看到別台裝置剛剛的變更 | ❌ 需要連線 |

更新的方式是「先給快取、背景抓新版」，所以改版後**第一次開啟還是舊版，下次開啟才會換新**。
抓到新版時右下角會提示一句。想立刻更新就把 App 關掉再開一次。

## 資料不會不見

這是設計時的重點，做了四層保護：

1. **雲端**：所有變更立刻寫進 Supabase，換裝置、重灌瀏覽器都還在。
2. **本機快取**：每次變更也存一份在瀏覽器。開啟時先顯示快取再跟雲端對時，
   所以就算當下沒網路或伺服器連不上，看到的仍是最後一次的完整資料，**不會變空白**。
3. **待上傳佇列**：離線時做的新增／修改／刪除會排隊，右上角顯示「待上傳 N」，
   一恢復連線就自動補送；此時關閉分頁瀏覽器會出聲警告。
4. **手動備份**：右上角「⋮」→ **匯出備份 JSON**，存成檔案自己留一份。

右上角那顆小圓點就是同步狀態：`已同步` / `同步中` / `待上傳 N` / `離線`。

---

## 先試用看看

不想馬上設定 Supabase，可以在網址後面加 `?demo=1`：

```
index.html?demo=1
```

會進入試用模式，資料只存在這台裝置的瀏覽器、不會同步，用來先看介面。
正式使用時把 `?demo=1` 拿掉即可（兩邊資料是分開的）。

---

## 疑難排解

| 狀況 | 原因與處理 |
|---|---|
| 一直停在設定畫面 | `CONFIG` 沒填，或 URL 打錯（要是 `https://xxx.supabase.co`，結尾不要有斜線） |
| 登入後跳「資料表尚未建立」 | 步驟二的 `schema.sql` 沒跑成功，回 SQL Editor 重跑一次 |
| 註冊後無法登入 | 沒關 Confirm email，去信箱點確認信；或做步驟三 |
| 右上角一直顯示「待上傳」 | 網路不通，或 `schema.sql` 的 RLS 沒建好。點「⋮」→ 立即重新同步 看有沒有錯誤訊息 |
| 手機看不到電腦新增的事項 | 確認兩邊登入的是同一個 Email；切回 App 時會自動重抓，也可手動「立即重新同步」 |
