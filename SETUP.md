...
> ⚠️ **2026-08-01 更新後要跑一次 migration**：新增了排程的起訖日、「自訂」頻率、
> 以及單一行程的多日活動。請到 Supabase → SQL Editor 執行
> [`migrations/2026-08-recurrence.sql`](migrations/2026-08-recurrence.sql)，
> 否則新增／修改排程會存不起來（App 偵測到會在頂端顯示紅色提示）。
...
- 右下角 **＋ 新增行程** 可加入單一行程（例如某天的演講、考試）。
  ...
  月曆上那三天都會出現，並標示「第 2/3 天」。
- 單一行程在月曆頁可以直接編輯與刪除。

**兩種項目在月曆上長得不一樣**：單一行程是圓角描邊的色塊（手機是菱形點），
