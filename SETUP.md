...
- **隨程式附的排程檔**（列在 `imports/index.json`），例如「台大一般外科 教學CR 職務排程」，點一下就匯入。
- **從檔案選擇**：可匯入自己「匯出備份 JSON」存下來的檔案，等同還原備份。

匯入前會列出每一筆讓你確認。項目依內部編號覆蓋，**重複匯入同一個檔案不會產生重複資料**，
所以檔案更新後再匯入一次就是更新內容。

要新增自己的排程檔，把 JSON 放進 `imports/`，格式為
`{"schedules":[…],"events":[…],"categories":[{"name":"…"}]}`（欄位同匯出備份），
再到 `imports/index.json` 加一列 `{"file":"…","title":"…","description":"…"}`。
